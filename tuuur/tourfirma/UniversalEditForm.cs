...
namespace tourfirma
{
    public partial class UniversalEditForm : Form
    {
        ...
        private void ConfigureForm()
        {
            ...
            switch (_activeTab)
            {
                case "tabPage2": // Объединенная информация о туристах
                    ConfigureTouristTab();
                    break;
        ...
        {
            // Исправляем названия полей, чтобы они соответствовали БД
            AddTextField("Фамилия*", "tourist_surname"); // было "surname"
            AddTextField("Имя*", "tourist_name");       // было "name"
            AddTextField("Отчество", "tourist_otch");   // было "patronymic"
        ...
        private void ConfigurePutevkiTab()
        {
            ...
            // Добавляем поле для изменения цены конкретной путевки
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (var transaction = _connection.BeginTransaction())
                {
                    ...
                    {
                        // Для путевок - дополнительная проверка на изменение цены
        ...
        private string GetUpdateSql(string tableName)
        {
            ...
            // Для путевок добавляем обновление цены
            if (tableName == "putevki")
            {
                setClause = setClause.Replace("price = @price", $"price = @price");
            }
